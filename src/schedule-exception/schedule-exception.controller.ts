import { 
  ...
  import { ExceptionTypeEnum, ScheduleExceptionSchemaClass } from './infrastructure/persistence/document/entities/schedule-exception.schema';
  ...
  export class ScheduleExceptionController {
    ...
    @ApiResponse({ status: 201, description: 'The schedule exception has been successfully created.' })
    async create(@Body() createExceptionDto: CreateScheduleExceptionDto, @Request() req) {
      ...
      if (!isAdmin && !isVendorOwner) {
        throw new UnauthorizedException('Not authorized to create schedule exceptions for this vendor');
      }
    ...
    @ApiResponse({ status: 200, description: 'Returns the schedule exception details.' })
    async findOne(@Param('id') id: string, @Request() req) {
      ...
      if (!isAdmin && !isVendorOwner) {
        throw new UnauthorizedException('Not authorized to view this schedule exception');
      }
    ...
    @ApiResponse({ status: 200, description: 'Returns all schedule exceptions for the vendor.' })
    async findByVendor(@Param('vendorId') vendorId: string, @Request() req) {
      ...
      if (!isAdmin && !isVendorOwner) {
        throw new UnauthorizedException('Not authorized to view schedule exceptions for this vendor');
      }
    ...
    @ApiResponse({ status: 200, description: 'Returns all schedule exceptions for the date.' })
    ...
    async findForDate(
      ...
      if (vendorId) {
        ...
        if (req.user.role?.id !== RoleEnum.admin) {
          throw new UnauthorizedException('Not authorized to view all schedule exceptions');
        }
    ...
    @ApiOperation({ summary: 'Get schedule exceptions for a vendor within a date range' })
    @ApiResponse({ status: 200, description: 'Returns exceptions within the date range.' })
    ...
    async findByDateRange(
      ...
      return this.scheduleExceptionService.findByDateRange(vendorId, startDate, endDate);
    }
  
    @Put(':id')
    @ApiOperation({ summary: 'Update a schedule exception' })
    @ApiResponse({ status: 200, description: 'The schedule exception has been successfully updated.' })
    async update(
      @Param('id') id: string,
      ...
      if (!isAdmin && !isVendorOwner) {
        throw new UnauthorizedException('Not authorized to update this schedule exception');
      }
    ...
    async remove(@Param('id') id: string, @Request() req) {
      ...
      if (!isAdmin && !isVendorOwner) {
        throw new UnauthorizedException('Not authorized to delete this schedule exception');
      }
    ...
    @Get('check-booking')
    @ApiOperation({ summary: 'Check if a booking time is affected by any exceptions' })
    @ApiResponse({ status: 200, description: 'Returns relevant exceptions if any.' })
...
@ApiResponse({ status: 201, description: 'The schedule exceptions have been successfully created.' })
async createBulk(@Body() createExceptionDtos: CreateScheduleExceptionDto[], @Request() req) {
  if (!Array.isArray(createExceptionDtos) || createExceptionDtos.length === 0) {
    throw new BadRequestException('Please provide an array of schedule exceptions');
  }
  
  // Check if user is authorized for all vendors
  const vendorIds = [...new Set(createExceptionDtos.map(dto => dto.vendorId))];
  ...
  for (const vendorId of vendorIds) {
    ...
    if (!isAdmin && !isVendorOwner) {
      throw new UnauthorizedException(`Not authorized to create schedule exceptions for vendor ${vendorId}`);
    }
  }
  
  // Fix: Properly type the results array
  const results: Array<{ data: ScheduleExceptionSchemaClass; message: string }> = [];
